...
#define immJ() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 20)| (BITS(i, 20, 20) << 11) | (BITS(i, 30, 21) << 1) | (BITS(i, 19, 12) << 12) | 0; } while(0)
...
#define ECALL(i) { bool success; i = (isa_raise_intr(isa_reg_str2val("a7", &success), s->pc)); }
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> (sword_t)imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | imm);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = CSR(imm); CSR(imm) = src1; R(rd) = t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = CSR(imm); printf("csrrs:0x%08x, src1:0x%08x, mstatus=0x%08x\n", t, src1, imm); CSR(imm) = t | src1; R(rd) = t);
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = ((SEXT((long long)src1, 32) * SEXT((long long)src2, 32)) >> 32));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm);//带符号位的比大小用sword_t
